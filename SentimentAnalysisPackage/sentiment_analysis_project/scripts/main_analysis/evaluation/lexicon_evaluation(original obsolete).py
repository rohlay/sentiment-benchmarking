...
from sentiment_analysis_project.scripts.main_analysis.config import GENERAL_INQUIRER_FILE, MPQA_FILE, OPINIONFINDER_FILE
from sentiment_analysis_project.scripts.main_analysis.config import DATA_PROCESSED
from sentiment_analysis_project.scripts.main_analysis.config import CONFIG_LEXICON
from sentiment_analysis_project.scripts.main_analysis.config import OUTPUTS_DIR
...
compute_metric_correctedness = False # Correctedness = 1 - RMSE
...
class LexiconEvaluation:
    ...
    def calculate_continuous_metrics_norm(self, y_true, y_pred): 
        ...
        # all elements in y_true, y_pred are bounded within [-1,1]
        # normalized metrics bounded by [0,1]
        max_error = 2
        mse_norm = mse / max_error**2
        mae_norm = mae / max_error
        rmse_norm = rmse / max_error
        r2_norm = (r2 + 1) / 2
        evs_norm = (evs + 1) / 2

        if compute_metric_correctedness:
            ...
            return {
                ...
                'r2_score': r2_norm,
                'explained_variance_score': evs_norm
            }
    ...
    def calculate_continuous_metrics_and_normalized(self, y_true, y_pred, data_range): # y_max, y_min
        ...
        metrics_norm = {
            'mean_squared_error': mse/(y_error_max),
            'mean_absolute_error': mae/(y_error_max),
            'root_mean_squared_error': rmse,
            'r2_score': r2_score,
            'explained_variance_score': evs}
    ...
    def map_ground_truth_values(self, y_true, data_range):
        ...
        mapped_ground_truth = new_low + (y_true - old_low) * (new_high - new_low) / (old_high - old_low)
    ...
    # Save csv
    """   
    def save_results_to_csv(self, results, original_res_dir):
        for result_type, datasets in results.items():
            for dataset_name, models in datasets.items():
                ...
                for metric in metrics:
                    file_path = os.path.join(original_res_dir, f"{metric}.csv")
    ...
    def main(self):
        ...
        for (dataset_name, dataset_file, language, data_range) in datasets: 
            ...
            for model in models:
                ...
                #metric_results, metric_results_normalized = self.calculate_continuous_metrics_and_normalized(y_true_mapped, predictions_mapped, data_range)
                metric_results_normalized = self.calculate_continuous_metrics_norm(y_true_mapped, predictions_mapped)
